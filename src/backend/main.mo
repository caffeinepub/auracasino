...
actor {
  public type UserProfile = { balance : Nat; totalWagered : Nat; totalWon : Nat };
  public type UserStatProfile = { principal : Text; balance : Nat; totalWagered : Nat; totalWon : Nat };
  public type UserCredential = { username : Text; password : Text; balance : Nat };
  public type GameStats = { totalWagered : Nat; totalPaidOut : Nat; playCount : Nat };
  public type PlayResult = { win : Bool; payout : Nat; result : Nat; message : Text };
  public type RouletteBet = { betType : Text; betValue : Nat; wager : Nat };
  public type MultiPlayResult = { win : Bool; totalPayout : Nat; result : Nat; message : Text };
  public type SlotsResult = { win : Bool; payout : Nat; reels : [Nat]; message : Text };
  public type HiLoResult = { win : Bool; payout : Nat; newCard : Nat; message : Text };
  public type AviatorResult = { win : Bool; payout : Nat; crashPoint : Nat; message : Text };
  public type TeenPattiResult = { win : Bool; payout : Nat; playerCards : [Nat]; dealerCards : [Nat]; playerRank : Nat; dealerRank : Nat; message : Text };
  public type AdminStats = { totalUsers : Nat; totalWagered : Nat; totalPaidOut : Nat; houseProfit : Int; rouletteStats : GameStats; slotsStats : GameStats; hiloStats : GameStats; aviatorStats : GameStats; teenPattiStats : GameStats };
  public type LoginResult = { success : Bool; balance : Nat; message : Text };
  public type PlayerWallet = { username : Text; balance : Nat };
  public type GameRecord = { username : Text; game : Text; wager : Nat; payout : Nat; win : Bool; timestamp : Int };
  ...
  system func postupgrade() {
    for ((k, v) in stableCreatedUsers.vals()) { createdUsers.add(k, v) };
    for ((k, v) in stablePlayerWallets.vals()) { playerWallets.add(k, v) };
  ...
  func addGameRecord(record : GameRecord) {
    ...
    gameHistory := Array.tabulate<GameRecord>(newSize, func(i) { if (i == 0) { record } else { current[i - 1] } });
  };

  func getUserOrRegister(caller : Principal) : UserProfile {
    switch (users.get(caller)) {
      case (?user) { user };
      case (null) { let newUser : UserProfile = { balance = initialBalance; totalWagered = 0; totalWon = 0 }; users.add(caller, newUser); newUser };
    };
  };

  func updateUserProfile(caller : Principal, profile : UserProfile) { users.add(caller, profile) };
  func updateGameStats(stats : GameStats, wager : Nat, payout : Nat) : GameStats { { totalWagered = stats.totalWagered + wager; totalPaidOut = stats.totalPaidOut + payout; playCount = stats.playCount + 1 } };

  func validatePlayer(username : Text, password : Text) : (Bool, Nat) {
    switch (createdUsers.get(username)) {
      case (null) { (false, 0) };
      case (?storedPw) {
        if (storedPw != password) { (false, 0) } else {
          let bal = switch (playerWallets.get(username)) { case (?b) { b }; case (null) { playerWallets.add(username, initialBalance); initialBalance } };
          (true, bal);
        };
      };
    };
  ...
  func getPlayerBalance(username : Text) : Nat {
    switch (playerWallets.get(username)) { case (?b) { b }; case (null) { initialBalance } };
  };

  func setPlayerBalance(username : Text, balance : Nat) { playerWallets.add(username, balance) };
  ...
  public shared func playerLogin(username : Text, password : Text) : async LoginResult {
    ...
    if (valid) { { success = true; balance; message = "Login successful" } }
    else if (createdUsers.containsKey(username)) { { success = false; balance = 0; message = "Incorrect password" } }
    else { { success = false; balance = 0; message = "Username not found" } };
  };
  ...
  public shared func playerPlayRouletteMulti(username : Text, password : Text, bets : [RouletteBet]) : async MultiPlayResult {
    ...
    totalWagered += totalBetWager; totalPaidOut += totalPayout;
  ...
  public shared func playerPlayAviator(username : Text, password : Text, wager : Nat, targetMultiplierX100 : Nat) : async AviatorResult {
    ...
    if (balance < wager) { return { win = false; payout = 0; crashPoint = 100; message = "Insufficient balance" } };
    ...
    let msg = if (win) { "Cashed out at " # (targetMultiplierX100 / 100).toText() # "." # (targetMultiplierX100 % 100).toText() # "x!" } else { "Crashed at " # (rng / 100).toText() # "." # (rng % 100).toText() # "x" };
  ...
  public shared func playerPlayTeenPatti(username : Text, password : Text, wager : Nat) : async TeenPattiResult {
    let (valid, balance) = validatePlayer(username, password);
    if (not valid) { Runtime.trap("Invalid credentials") };
    if (balance < wager) { return { win = false; payout = 0; playerCards = [0, 0, 0]; dealerCards = [0, 0, 0]; playerRank = 0; dealerRank = 0; message = "Insufficient balance" } };
    let p1 = await Random.natRange(0, 52); let p2 = await Random.natRange(0, 52); let p3 = await Random.natRange(0, 52);
    let d1 = await Random.natRange(0, 52); let d2 = await Random.natRange(0, 52); let d3 = await Random.natRange(0, 52);
    ...
    let handName = func(r : Nat) : Text { if (r == 5) { "Trail" } else if (r == 4) { "Pure Sequence" } else if (r == 3) { "Sequence" } else if (r == 2) { "Color" } else if (r == 1) { "Pair" } else { "High Card" } };
    let msg = if (win) { "You win! " # handName(playerRank) # " beats " # handName(dealerRank) } else { "Dealer wins. " # handName(dealerRank) # " beats " # handName(playerRank) };
  ...
  public query func adminGetPlayerWallets() : async [PlayerWallet] {
    createdUsers.toArray().map(func((username, _)) : PlayerWallet { let balance = switch (playerWallets.get(username)) { case (?b) { b }; case (null) { 0 } }; { username; balance } });
  };

  public shared func adminAdjustBalance(username : Text, amount : Nat, isAdd : Bool) : async Text {
    if (not createdUsers.containsKey(username)) { return "User not found" };
    let current = getPlayerBalance(username);
    let newBalance = if (isAdd) { current + amount } else { if (amount >= current) { 0 } else { current - amount } };
  ...
  public query func adminGetCreatedUsers() : async [Text] {
    createdUsers.toArray().sort(func(a, b) { Text.compare(a.0, b.0) }).map(func((u, _)) { u });
  };

  public query func adminGetGameHistory() : async [GameRecord] { gameHistory };
  ...
  public shared ({ caller }) func register() : async () {
    ...
    switch (users.get(caller)) {
      ...
      case (null) { users.add(caller, { balance = initialBalance; totalWagered = 0; totalWon = 0 }) };
    };
  };
  ...
  public shared ({ caller }) func playAviator(wager : Nat, targetMultiplierX100 : Nat) : async AviatorResult {
    ...
    if (targetMultiplierX100 < 110) { Runtime.trap("Minimum cashout is 1.10x") };
    let user = getUserOrRegister(caller);
    if (user.balance < wager) { return { win = false; payout = 0; crashPoint = 100; message = "Insufficient balance" } };
    let rng = await Random.natRange(100, 1001);
    let win = targetMultiplierX100 <= rng;
    let payout = if (win) { wager * targetMultiplierX100 / 100 } else { 0 };
    updateUserProfile(caller, { balance = user.balance - wager + payout; totalWagered = user.totalWagered + wager; totalWon = user.totalWon + payout });
  ...
  func teenPattiHandRank(c1 : Nat, c2 : Nat, c3 : Nat) : Nat {
    let r1 = cardRank(c1); let r2 = cardRank(c2); let r3 = cardRank(c3);
    let s1 = cardSuit(c1); let s2 = cardSuit(c2); let s3 = cardSuit(c3);
    ...
    if (isTrail) { 5 } else if (isFlush and isSeq) { 4 } else if (isSeq) { 3 } else if (isFlush) { 2 } else if (isPair) { 1 } else { 0 };
  };

  func teenPattiHighCard(c1 : Nat, c2 : Nat, c3 : Nat) : Nat { maxNat(cardRank(c1), maxNat(cardRank(c2), cardRank(c3))) };

  public shared ({ caller }) func playTeenPatti(wager : Nat) : async TeenPattiResult {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) { Runtime.trap("Unauthorized") };
    let user = getUserOrRegister(caller);
    if (user.balance < wager) { return { win = false; payout = 0; playerCards = [0,0,0]; dealerCards = [0,0,0]; playerRank = 0; dealerRank = 0; message = "Insufficient balance" } };
    let p1 = await Random.natRange(0, 52); let p2 = await Random.natRange(0, 52); let p3 = await Random.natRange(0, 52);
    let d1 = await Random.natRange(0, 52); let d2 = await Random.natRange(0, 52); let d3 = await Random.natRange(0, 52);
    let playerRank = teenPattiHandRank(p1, p2, p3);
    let dealerRank = teenPattiHandRank(d1, d2, d3);
    ...
    teenPattiStats := updateGameStats(teenPattiStats, wager, payout);
    let handName = func(r : Nat) : Text { if (r == 5) { "Trail" } else if (r == 4) { "Pure Sequence" } else if (r == 3) { "Sequence" } else if (r == 2) { "Color" } else if (r == 1) { "Pair" } else { "High Card" } };
    let msg = if (win) { "You win! " # handName(playerRank) } else { "Dealer wins. " # handName(dealerRank) };
    { win; payout; playerCards = [p1, p2, p3]; dealerCards = [d1, d2, d3]; playerRank; dealerRank; message = msg };
  };
  ...
  public shared ({ caller }) func playSlots(wager : Nat) : async SlotsResult {
    ...
    let rng1 = await Random.natRange(0, 7); let rng2 = await Random.natRange(0, 7); let rng3 = await Random.natRange(0, 7);
  ...
  public query func adminGetAllUsers() : async [UserStatProfile] {
    users.entries().toArray().map(func((principal, profile)) : UserStatProfile { { principal = principal.toText(); balance = profile.balance; totalWagered = profile.totalWagered; totalWon = profile.totalWon } });
  };

  public query func adminGetStats() : async AdminStats {
    { totalUsers = users.size(); totalWagered; totalPaidOut; houseProfit = (totalWagered : Int) - (totalPaidOut : Int); rouletteStats; slotsStats; hiloStats; aviatorStats; teenPattiStats };
  };

  public query func adminGetUsersWithPasswords() : async [UserCredential] {
    createdUsers.toArray().map(func((username, password)) : UserCredential {
      let balance = switch (playerWallets.get(username)) { case (?b) { b }; case (null) { 0 } };
